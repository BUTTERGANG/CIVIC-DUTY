...
router.get('/', async (req, res) => {
  try {
    ...
    let query = 'SELECT * FROM visionzero_crashes WHERE 1=1';
    const params: any[] = [];

    if (city) { params.push(city); query += ` AND city = $${params.length}`; }
    if (crash_type) { params.push(crash_type); query += ` AND crash_type = $${params.length}`; }
    ...
    sendError(res, err, 'VisionZero');
  }
});

router.get('/:id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM visionzero_crashes WHERE id = $1', [req.params.id]);
